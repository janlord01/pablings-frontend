<template>
  <q-card style="max-width: 800px; width: 800px; min-height: 400px">
    ...
    <q-toolbar class="bg-blue text-white">
      <q-toolbar-title> Update Service </q-toolbar-title>
          ...
          <div :class="$q.screen.gt.xs ? 'col-5 q-mr-sm' : 'full-width'">
            ...
            >
              <template v-slot:before>
                <q-avatar>
                  <img
                    v-if="formData.image == oldImage"
                    :src="formData.image"
                  />
                  <q-icon v-else name="add_photo_alternate" size="20px" />
                </q-avatar>
              </template>
...
const getServiceDetails = async () => {
  ...
  await api
    .get(`/api/services/${props.serviceId}/edit`, {
      headers: {
    ...
    .then((response) => {
      ...
      formData.name = response.data.data.service_name;
      formData.price = response.data.data.service_price;
      formData.timeDuration = response.data.data.time_duration;
      formData.description = response.data.data.service_descr;
      formData.image = response.data.data.image;
      oldImage.value = response.data.data.image;
    })
    .catch((error) => {
      console.log(error);
    });
