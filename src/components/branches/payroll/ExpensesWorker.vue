<template>
  <q-table
    ...
    :rows="payrollStore.ExpensesData"
    :columns="columns"
    :loading="payrollStore.loadingExpenses"
    title="Deduction/Loan"
    row-key="id"
    separator="horizontal"
    :visible-columns="
      $q.screen.gt.xs
        ? ['transaction', 'item', 'expenses', 'action']
        : ['transaction', 'item', 'expenses', 'action']
    "
  >
      ...
      <q-tr :props="props">
        ...
        <q-td key="action" :props="props">
          <q-btn
            ...
            icon="delete"
            size="sm"
            @click="payrollStore.removeItem([props.row.uid, 'expense'])"
          />
        </q-td>
      </q-tr>
    </template>
