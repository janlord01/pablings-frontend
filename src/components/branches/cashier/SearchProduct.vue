<template>
  <q-card style="max-width: 720px; width: 720px; min-height: 400px">
    ...
    <q-toolbar class="bg-primary text-white">
      <q-toolbar-title> Search Product </q-toolbar-title>
        ...
        <div class="row q-col-gutter-none relative-position">
          ...
          <div class="row relative-position q-mt-lg">
            <q-card
              ...
              v-for="item in productStore.rowDatas"
              :key="item.id"
            >
              <img
                :src="item.product_img"
                width="100"
                v-if="item.product_img != null"
              />
              ...
              <q-card-section>
                ...
                <div class="text-subtitle2">
                  <span v-if="item.sale === 'true'"
                    ><span class="text-strike" style="margin-right: 2px"
                      >P{{ item.price }}</span
                    >P{{ item.sale_price }}</span
                  >
                  <span v-else> P{{ item.price }}</span
                  ><br />
...
const addProduct = async (product_id) => {
  ...
  if (a >= 1) {
    ...
    await api
      .get(
        "/api/products/add/product/" + product_id,
        {
          params: {
      ...
      .then((response) => {
        ...
        if (response.data.status == 404) {
          ...
          productStore.addProductCashier(response);
        }
...
const searchMemberFunc = () => {
  ...
  productStore.onSearch([name, route.params.id]);
  //   console.log(name);
};
